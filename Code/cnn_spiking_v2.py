...
# Crear subconjuntos para dividir los datos en lotes más pequeños
def create_batches(data, labels, batch_size):
    dataset = TensorDataset(data, labels)
    indices = list(range(len(dataset)))
    np.random.shuffle(indices)
    batches = [Subset(dataset, indices[i:i + batch_size]) for i in range(0, len(dataset), batch_size)]
    return batches

batch_size = 1024  # Ajusta el tamaño del lote según tu capacidad de memoria
    ...
    with tqdm(total=total_steps, desc="Training Progress", unit="step") as pbar:
        for epoch in range(num_epochs):
            for batch in train_batches:
                train_loader = DataLoader(batch, batch_size=32, shuffle=True)
...
with torch.no_grad():
    for batch in val_batches:
        val_loader = DataLoader(batch, batch_size=32, shuffle=False)
...
with torch.no_grad():
    for batch in test_batches:
        test_loader = DataLoader(batch, batch_size=32, shuffle=False)
